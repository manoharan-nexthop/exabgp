...
class SRPolicyNLRI(NLRI):
    ...
    @classmethod
    def create(cls, afi: AFI, distinguisher: int, color: int, endpoint: str, action: Action = Action.ANNOUNCE) -> 'SRPolicyNLRI':
    ...
    def pack_nlri(self, negotiated: Negotiated) -> Buffer:
        ...
        Length = size of Distinguisher + Color + Endpoint (12 for IPv4, 24 for IPv6)
        """
        return bytes([len(self._packed)]) + self._packed
    ...
    @classmethod
    def unpack_nlri(
        cls, afi: AFI, safi: SAFI, data: Buffer, action: Action, addpath: Any
    ) -> tuple[NLRI, Buffer]:
        ...
        # Validate length
        if nlri_len != expected:
            raise Notify(3, 10, f'SR Policy NLRI invalid length: got {nlri_len}, expected {expected} for AFI {afi}')

        # Check we have enough data
        if len(data) < 1 + expected:
            raise Notify(3, 10, f'SR Policy NLRI too short: need {1 + expected} bytes, got {len(data)}')

        # Skip length byte, extract NLRI data
        nlri = cls(afi, data[1:1+expected], action)
        nlri.addpath = addpath
        return nlri, data[1+expected:]
