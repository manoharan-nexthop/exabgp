...
def _parse_segment_list(tokeniser: Any) -> SegmentListSubTLV:
    ...
    segments: list[SegmentTypeA | SegmentTypeB | SegmentTypeC | SegmentTypeF | SegmentTypeG | SegmentTypeH | SegmentTypeI | SegmentTypeJ | SegmentTypeK] = []
    ...
    while tokeniser.peek() == 'segment':
        ...
        if seg_type == 'type-a':
            ...
            if tokeniser.peek() == 'sid':
                tokeniser()  # consume 'sid'
                sid = int(tokeniser())
            segments.append(SegmentTypeG(local_if_id=local_if_id, local_ipv6=local_ipv6, remote_if_id=remote_if_id, remote_ipv6=remote_ipv6, sid=sid))
            ...
            if eb is not None:
                flags |= 0x10  # B-Flag
            segments.append(SegmentTypeI(ipv6_node=ipv6_node, algorithm=algorithm, flags=flags, sid=sid, endpoint_behavior=eb))
            ...
            if eb is not None:
                flags |= 0x10  # B-Flag
            segments.append(SegmentTypeJ(local_if_id=local_if_id, local_ipv6=local_ipv6, remote_if_id=remote_if_id, remote_ipv6=remote_ipv6, algorithm=algorithm, flags=flags, sid=sid, endpoint_behavior=eb))
            ...
            if eb is not None:
                flags |= 0x10  # B-Flag
            segments.append(SegmentTypeK(local_ipv6=local_ipv6, remote_ipv6=remote_ipv6, algorithm=algorithm, flags=flags, sid=sid, endpoint_behavior=eb))
        else:
            raise ValueError(f"Unknown segment type '{seg_type}'. Expected: type-a, type-b, type-c, type-d, type-e, type-f, type-g, type-h, type-i, type-j, type-k")
