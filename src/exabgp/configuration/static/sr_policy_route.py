...
# Import standard BGP attribute parsers
...
def sr_policy(tokeniser):
    ...
    while True:
        ...
        if command == 'next-hop':
            nexthop = result
        elif command in ('preference', 'priority', 'binding-sid', 'srv6-binding-sid',
                        'policy-name', 'candidate-path-name', 'segment-list'):
            # SR-Policy sub-TLVs
...
def segment_type_g(tokeniser):
    ...
    return SegmentTypeG(local_if_id=local_if_id, local_ipv6=local_ipv6, remote_if_id=remote_if_id, remote_ipv6=remote_ipv6, sid=sid)
...
def segment_type_j(tokeniser):
    ...
    return SegmentTypeJ(local_if_id=local_if_id, local_ipv6=local_ipv6, remote_if_id=remote_if_id, remote_ipv6=remote_ipv6, algorithm=algorithm, flags=flags, sid=sid, endpoint_behavior=eb)
...
def segment_type_k(tokeniser):
    ...
    return SegmentTypeK(local_ipv6=local_ipv6, remote_ipv6=remote_ipv6, algorithm=algorithm, flags=flags, sid=sid, endpoint_behavior=eb)
    ...
    def parse(self, name, command):
        ...
        if command == 'segment':
            ...
            if seg_type == 'type-a':
                ...
                return self.error.set(f"Unknown segment type '{seg_type}'. Expected: type-a, type-b, type-c, type-f, type-g, type-h, type-i, type-j, type-k")
            return True
        else:
